...
from models.model_kp import AssociationModel
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(tqdm(train_dataloader)):
            ...
            loss, results, _, _ = criterion(sim_matrix, order, mask_cam, mask_imu)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        test_scene, test_matches, test_orders, test_masks = [], [], [], []
        with torch.no_grad():
            for i, data in enumerate(tqdm(test_dataloader)):
                ...
                if args.hands == 'right':
                    ...
                    sim_matrix = model(box, arm_left, imu_left)
                loss, results, match, mask_cam_valid = criterion(sim_matrix, order, mask_cam, mask_imu)
                test_loss += loss.item()
                ...
                test_scene.append(scenario.cpu().tolist())
                test_matches.append(match.detach().cpu().tolist())
                test_orders.append(order.detach().cpu().tolist())
                test_masks.append(mask_cam_valid.detach().cpu().tolist())
        ...
        if test_precision/len(test_dataloader) > best_test_precision:
            ...
            with open(f'checkpoints/CrossUser/{args.name}_best_results.json', 'w') as f:
...
def parse_args():
    ...
    parser.add_argument('--name', type=str, default='Exp_CrossU', help='Experiment name')
    parser.add_argument('--data_path', type=str, default='../User Study Data/Synth/CrossU')
...
if __name__ == '__main__':
    ...
    args.cuda = 1
    args.window_size = 60
    args.name = f'{args.name}{args.num_users}_t={args.window_size}_{args.hands}'
    # args.name = f'{args.name}_t={args.window_size}_{args.hands}'
    main(args)
